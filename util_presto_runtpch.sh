...
if [[ "$1" =~ ^[0-9]+$ && "$1" -gt "1" ]]; then
    ...
    # query file name
    QUERY_BASE_NAME="tpch_presto/tpch_query"
    QUERY_FILE_EXT=".hql"
    ...
    for (( i = $START; i <= $END; i++ )); do
        ...
        if [[ -f $query_path ]]; then
            ./util_prestoRunQuery.sh "$DATABASE" "$CURR_DIR$SETTINGS_PATH" "$CURR_DIR$query_path" "$CURR_DIR$LOG_PATH" "$i" "$CURR_DIR$REPORT_NAME.csv"
